...
- Optional local PIN lock with five-minute inactivity locking, Android biometric unlock when supported, early Android `FLAG_SECURE` protection, neutral Android task identity until intentional entry, Windows packaged content protection, neutral later-startup surfaces, and a clear privacy boundary
...
- Branded application identity using `logo/icon.png` for the app icon and `logo/logo.png` for the first-launch branded splash; later Discreet Mode launches use a neutral startup mark
